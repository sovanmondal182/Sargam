module.exports = {
    ...
    execute(client, message, args) {
        ...
        if (!args[0]) return message.channel.send({
            embed: {
                color: 'RANDOM',
                description: `${client.emotes.error} - Please specify a valid filter to enable or disable !`},
            });
        ...
        if (!filterToUpdate) return message.channel.send({
            embed: {
                color: 'RANDOM',
                description: `${client.emotes.error} - This filter doesn't exist, try for example (8D, vibrato, pulsator...) !`},
            });
        ...
        if (filtersUpdated[filterToUpdate]) message.channel.send({
            embed: {
                color: 'RANDOM',
                description: `${client.emotes.music} - I'm **Adding** the filter to the music, please wait... \n**Note :** The longer the music is, the longer this will take.`},
            });
        else message.channel.send({
            embed: {
                color: 'RANDOM',
                description: `${client.emotes.music} - I'm **Disabling** the filter on the music, please wait... \n**Note :** The longer the music is playing, the longer this will take.`},
            });
    },
};