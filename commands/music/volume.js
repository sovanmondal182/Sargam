module.exports = {
    ...
    execute(client, message, args) {
        ...
        if (!args[0] || isNaN(args[0]) || args[0] === 'Infinity') return message.channel.send({
            embed: {
                color: 'RANDOM',
                description: `${client.emotes.error} - Please enter a valid number !`,
            },
    });
        if (Math.round(parseInt(args[0])) < 1 || Math.round(parseInt(args[0])) > 300) return message.channel.send({
            embed: {
                color: 'RANDOM',
                description: `${client.emotes.error} - Please enter a valid number (between 1 and 300) !`,
            },
    });
